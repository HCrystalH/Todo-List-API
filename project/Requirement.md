...
* Use a database to store the user and to-do list data (can use any DB)
...
`
this will validate the given details, make sure the email is unique and store the user details in DB.

Make sure to hash the password before storing it in the DB. Respond with a token that can be used for authentication if the registration is successful.

`{
    "token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9"
}`

The token can either be a JWT or a random string that can be used for authentication. We leave it up to you to decide the implementations details.
...
**CREATE A TO-DO ITEM**
Create a new to-do itme using the following request:
POST /todos
`{
    "title": "Buy groceries",
...
`
User must send token received from the login endpoint in the header to authenticate the request. You can use the 'Authoriation' header with the token as the value. In case the token is missing or valid, 
respond with an error and status code 401.
`{
...
Just like the create todo endpoint, user must send the token received. Also make sure to validiate the user has the permission to update the to-do item i.e the user is the creator of todo ittem that they are updating. 
